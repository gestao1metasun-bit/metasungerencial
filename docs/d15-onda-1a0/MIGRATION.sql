-- =====================================================================
-- D15 — Onda 1.A.0 (REV2) — Alinhamento Financeiro + Camada de Integrabilidade
-- Modo: DDL puro. Nenhum dado migrado. Nenhuma flag D15 ativada.
-- Escopo: financeiro gerencial alinhado ao charter + base estrutural
--          para integração futura com sistema fiscal/contábil EXTERNO.
-- NÃO implementa: SPED/ECD/ECF/EFD, escrituração, partidas dobradas,
--                 apuração tributária, fechamento contábil, motor NF-e,
--                 conector externo. Apenas estrutura de dados.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 0. PERMISSÕES NOVAS (enum app_permission)
-- ---------------------------------------------------------------------

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid
                 WHERE t.typname='app_permission' AND e.enumlabel='integracao.visualizar') THEN
    ALTER TYPE public.app_permission ADD VALUE 'integracao.visualizar';
  END IF;
END $$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid
                 WHERE t.typname='app_permission' AND e.enumlabel='integracao.mapear') THEN
    ALTER TYPE public.app_permission ADD VALUE 'integracao.mapear';
  END IF;
END $$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid
                 WHERE t.typname='app_permission' AND e.enumlabel='integracao.exportar') THEN
    ALTER TYPE public.app_permission ADD VALUE 'integracao.exportar';
  END IF;
END $$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid
                 WHERE t.typname='app_permission' AND e.enumlabel='integracao.reprocessar') THEN
    ALTER TYPE public.app_permission ADD VALUE 'integracao.reprocessar';
  END IF;
END $$;
...
-- ---------------------------------------------------------------------
-- 2. CAMPOS UNIVERSAIS DE INTEGRABILIDADE (mesma estrutura em 7 tabelas)
--    Charter: ERP gerencial integrável com sistema fiscal/contábil externo.
-- ---------------------------------------------------------------------

-- macro: function imutável que recebe nome de tabela via SQL inline
-- (mantemos como ALTER explícito para evitar dynamic SQL desnecessário)

-- 2.1 titulos_financeiros
ALTER TABLE public.titulos_financeiros
...
-- 2.8 CHECK comum em status_integracao (uma vez por tabela, idempotente)
DO $$
DECLARE
  t text;
BEGIN
  FOR t IN SELECT unnest(ARRAY['titulos_financeiros','parcelas_financeiras',
    ...
    EXECUTE format(
      'ALTER TABLE public.%I DROP CONSTRAINT IF EXISTS chk_%I_status_integracao;',
      t, t);
    EXECUTE format(
...
--    Sem motor, sem conector, sem fiscal. Apenas estrutura de dados.
-- ---------------------------------------------------------------------

-- 6.1 lotes_integracao
...
-- FK tardia: titulos.lote_integracao_id → lotes_integracao
...
-- 6.2 mapeamentos_externos (de-para genérico chave→valor por sistema/tipo)
...
-- 6.3 eventos_pendentes_integracao (fila lógica — não-processada nesta onda)
...
-- ---------------------------------------------------------------------
-- 7. SEM RPC. SEM VIEW. SEM DADOS. SEM FLAGS.
--    Onda 1.A (reescrita) é quem cria v_lancamentos_derivados e RPCs.
-- ---------------------------------------------------------------------
