...
CREATE OR REPLACE FUNCTION public.fn_audit_lancamento(
  _modulo text, _entidade text, _entidade_id uuid, _acao text,
  _valor_novo jsonb, _motivo text DEFAULT NULL
) RETURNS void
...
SELECT
  t.id AS lancamento_id, 'TITULO'::text AS origem, t.id AS origem_id,
  t.tipo AS tipo,
  CASE WHEN t.tipo = 'RECEBER' THEN 'ENTRADA' ELSE 'SAIDA' END AS direcao,
  t.competencia AS data_competencia, t.vencimento AS data_vencimento,
  NULL::date AS data_realizacao,
  t.valor_bruto, t.valor_liquido, t.saldo, t.status,
  t.cliente_id, t.fornecedor_id, t.contrato_id, NULL::uuid AS obra_id,
...
UNION ALL
SELECT
  r.id, 'RESCISAO', r.id, 'RESCISAO', 'SAIDA',
...
COMMENT ON VIEW public.v_lancamentos_derivados IS
  'D15 Onda 1.A - verdade derivada do financeiro. Nao materializada; security_invoker; respeita RLS de cada fonte.';

GRANT SELECT ON public.v_lancamentos_derivados TO authenticated;

CREATE OR REPLACE FUNCTION public.rpc_lancamento_criar(
  _tipo text, _origem_tipo text, _origem_id uuid, _valor numeric, _vencimento date,
  _natureza_id uuid, _centro_id uuid DEFAULT NULL, _conta_id uuid DEFAULT NULL,
  _cliente_id uuid DEFAULT NULL, _fornecedor_id uuid DEFAULT NULL,
  _contrato_id uuid DEFAULT NULL, _competencia date DEFAULT NULL,
  _observacoes text DEFAULT NULL, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _titulo uuid; _parc uuid; _exist jsonb;
BEGIN
  ...
  PERFORM public.fn_audit_lancamento('financeiro','titulo', _titulo, 'CRIAR',
    jsonb_build_object('tipo',_tipo,'valor',_valor,'vencimento',_vencimento,'parcela_id',_parc), _observacoes);
  ...
  IF _request_id IS NOT NULL THEN
    ...
    VALUES (_request_id,'rpc_lancamento_criar',_uid,_exist) ON CONFLICT (request_id) DO NOTHING;
  END IF;
  RETURN _exist;
END $$;
...
CREATE OR REPLACE FUNCTION public.rpc_titulo_baixar(
  _parcela_id uuid, _valor numeric, _data timestamptz, _conta_id uuid,
  _forma text, _observacao text DEFAULT NULL, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _tit uuid; _saldo numeric; _mov uuid; _exist jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'auth required' USING ERRCODE='28000'; END IF;
  ...
  PERFORM public.fn_audit_lancamento('financeiro','movimentacao', _mov, 'BAIXA',
    jsonb_build_object('parcela_id',_parcela_id,'titulo_id',_tit,'valor',_valor,'conta_id',_conta_id), _observacao);
...
CREATE OR REPLACE FUNCTION public.rpc_titulo_estornar(
  _movimentacao_id uuid, _motivo text, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _orig RECORD; _novo uuid; _exist jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'auth required' USING ERRCODE='28000'; END IF;
  IF NOT public.has_permission(_uid,'financeiro.movimentar') THEN
  ...
  PERFORM public.fn_audit_lancamento('financeiro','movimentacao', _novo, 'ESTORNO',
...
CREATE OR REPLACE FUNCTION public.rpc_titulo_cancelar(
  _titulo_id uuid, _motivo text, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _tit RECORD; _exist jsonb;
BEGIN
...
CREATE OR REPLACE FUNCTION public.rpc_adiantamento_registrar(
  _direcao text, _valor numeric, _data date, _conta_id uuid,
  _cliente_id uuid DEFAULT NULL, _fornecedor_id uuid DEFAULT NULL,
  _contrato_id uuid DEFAULT NULL, _competencia date DEFAULT NULL,
  _observacao text DEFAULT NULL, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _ad uuid; _exist jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'auth required'; END IF;
  IF NOT public.has_permission(_uid,'financeiro.movimentar') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE='42501';
...
CREATE OR REPLACE FUNCTION public.rpc_adiantamento_abater(
  _adiantamento_id uuid, _parcela_id uuid, _valor numeric,
  _observacao text DEFAULT NULL, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _ad RECORD; _parc RECORD; _abat uuid; _mov uuid; _exist jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'auth required'; END IF;
  IF NOT public.has_permission(_uid,'financeiro.movimentar') THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE='42501';
...
CREATE OR REPLACE FUNCTION public.rpc_renegociacao_aplicar(
  _titulo_origem_id uuid, _novo_valor numeric, _novo_vencimento date,
  _motivo text, _request_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _orig RECORD; _novo uuid; _parc uuid; _exist jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'auth required'; END IF;
  ...
  INSERT INTO public.titulos_financeiros(
    ...
    'Renegociacao de ' || COALESCE(_orig.codigo,_orig.id::text) || ': ' || _motivo, _uid
  ) RETURNING id INTO _novo;
  INSERT INTO public.parcelas_financeiras(titulo_id, numero, valor, saldo, vencimento, status, created_by)
