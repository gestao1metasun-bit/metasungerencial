-- D20.SUP.2.CLASS — Classificação obrigatória da requisição

-- 1) Flag de destino interno (almoxarifado)
ALTER TABLE public.suprimentos_requisicoes
  ADD COLUMN IF NOT EXISTS destino_almoxarifado boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_sup_req_almox
  ON public.suprimentos_requisicoes(destino_almoxarifado)
  WHERE deleted_at IS NULL;

-- 2) Seeds idempotentes ALMOXARIFADO em centros_custo e centros_resultado
INSERT INTO public.centros_custo (codigo, nome, tipo, ativo)
SELECT 'ALMOXARIFADO', 'Almoxarifado', 'GERAL', true
WHERE NOT EXISTS (SELECT 1 FROM public.centros_custo WHERE codigo = 'ALMOXARIFADO');

INSERT INTO public.centros_resultado (codigo, nome, tipo, ativo)
SELECT 'ALMOXARIFADO', 'Almoxarifado', 'ambos', true
WHERE NOT EXISTS (SELECT 1 FROM public.centros_resultado WHERE codigo = 'ALMOXARIFADO');

-- 3) RPC reescrita com validações de classificação
...
BEGIN
  ...
  IF v_destino_almox THEN
    -- Almoxarifado: ignora vínculo operacional e força CC/CR ALMOXARIFADO
